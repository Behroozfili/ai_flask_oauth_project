...
def test_auth(app, client):
    mock_token_response = {'access_token': 'fake-access-token', 'id_token': 'fake-id-token', 'userinfo': {'sub': '123', 'name': 'Test User'}}
    mock_user_info = {'sub': '12345', 'name': 'Test User', 'email': 'test@example.com'}
    ...
    with app.test_request_context(): 
        expected_redirect_url = url_for('root', _external=False)
        assert response.location.endswith(expected_redirect_url)
